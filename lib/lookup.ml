...
let iff_list t ~state ~empty_res ~precedent fact_and_entries
  : Fact_pattern.t Query_result.t
  =
  match fact_and_entries with
  | [] -> Found empty_res
  | fact_and_entries ->
    Hashtbl.add_exn (state t) ~key:precedent ~data:fact_and_entries;
